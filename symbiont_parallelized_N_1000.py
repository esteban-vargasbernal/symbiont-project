...
def process_simulation_task(N,c,M,x_alive,D_cell,D_sym,W_in, W_out, N_gen,s_for_symbionts, s_for_cells, N_sim, h_for_symbionts, e_for_symbionts, h_for_cells, e_for_cells, epsilon_cell, epsilon_sym, gamma_cell, gamma_sym):
    ...
    df = many_simulations_with_mutations(N,c,M,x_alive,x_cell,x_sym_in,x_sym_out,D_cell,D_sym,W_in, W_out, x_mut, N_gen,N_sim)
...
if __name__ == "__main__":
    ...
    h_for_cells_ls = 4*n_args*[0.25]
    e_for_symbionts_ls = 4*list(e_pairs[:,0])

    h_for_symbionts_ls = 4*n_args*[0.25]
    e_for_cells_ls = 4*list(e_pairs[:,1])

    N_sim_ls = 4*n_args*[N_sim]
    epsilon_cell_ls = n_args*[epsilon_cell_large] + n_args*[epsilon_cell_large] + n_args*[epsilon_cell_small] + n_args*[epsilon_cell_small]
    epsilon_sym_ls = n_args*[epsilon_sym_large] + n_args*[epsilon_sym_small] + n_args*[epsilon_sym_large] + n_args*[epsilon_sym_small]
    gamma_cell_ls = 4*n_args*[gamma_cell]
    gamma_sym_ls = 4*n_args*[gamma_sym]
    
    # change n_args at the end of the next line if needed 
    args = [(N,c,M,x_alive,D_cell,D_sym,W_in, W_out, N_gen,s_for_symbionts, s_for_cells, N_sim_ls[i], h_for_symbionts_ls[i], e_for_symbionts_ls[i], h_for_cells_ls[i], e_for_cells_ls[i], epsilon_cell_ls[i], epsilon_sym_ls[i], gamma_cell_ls[i], gamma_sym_ls[i]) for i in np.arange(4*n_args)]
