...
def mutation(N,c,M,x_alive, x_cell, x_sym_in,x_sym_out,x_mut):
    ...
    for i in np.arange(n_mut_cell):
        ...
        if i_trait == 1:
            x_cell_copy['h'][i_mutants[i],0] = x_cell['h'][i_mutants[i],0] + delta_cell[i,0]
        if i_trait == 2:
            x_cell_copy['e'][i_mutants[i],0] = x_cell['e'][i_mutants[i],0] + delta_cell[i,0]
    ...
    for i in np.arange(n_mut_in):
        ...
        if i_trait == 1:
            x_sym_in_copy['h'][i_mutants[i],j_mutants[i]] =  x_sym_in['h'][i_mutants[i],j_mutants[i]] + delta_in[i,0]
        if i_trait == 2:    
            x_sym_in_copy['e'][i_mutants[i],j_mutants[i]] =  x_sym_in['e'][i_mutants[i],j_mutants[i]] + delta_in[i,0]
    ...
    for i in np.arange(n_mut_out):
        ...
        if i_trait == 1:
            x_sym_out_copy['h'][0,i_mutants_out[i]] = x_sym_out_copy['h'][0,i_mutants_out[i]] + delta_out[i,0]
        if i_trait == 2:
            x_sym_out_copy['e'][0,i_mutants_out[i]] = x_sym_out_copy['e'][0,i_mutants_out[i]] + delta_out[i,0]
...
def process_simulation_task(N,c,M,x_alive,D_cell,D_sym,W_in, W_out, N_gen_max, R, s_for_symbionts, s_for_cells, N_sim, h_for_symbionts, e_for_symbionts, h_for_cells, e_for_cells, epsilon_cell, epsilon_sym, gamma_cell, gamma_sym):
    ...
    dfs = many_simulations_within_circle(N,c,M,x_alive,x_cell,x_sym_in,x_sym_out,D_cell,D_sym,W_in, W_out, x_mut, N_gen_max,R,N_sim)
...
if __name__ == "__main__":
    ...
    n_split = 2
    e_sym_tmp = np.linspace(-0.75,0.25,n_split)
    e_cell_tmp = np.linspace(-0.75,0.25,n_split)
    ...
    args = [(N,c,M,x_alive,D_cell,D_sym,W_in, W_out, N_gen_max,R,s_for_symbionts, s_for_cells, N_sim_ls[i], h_for_symbionts_ls[i], e_for_symbionts_ls[i], h_for_cells_ls[i], e_for_cells_ls[i], epsilon_cell_ls[i], epsilon_sym_ls[i], gamma_cell_ls[i], gamma_sym_ls[i]) for i in np.arange(n_args)]
