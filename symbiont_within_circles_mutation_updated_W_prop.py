...
def update_traits_after_death(N,c,M,x_alive_new,x_cell,x_sym_in,x_sym_out):
    ...
    x_cell_new = dict(s = s_cell, h = h_cell, e = e_cell, w = w_cell)
    x_sym_in_new = dict(s = s_sym_in, h = h_sym_in, e = e_sym_in)  #, w = w_sym_in)
    x_sym_out_new = dict(s = s_sym_out, h = h_sym_out, e = e_sym_out) #, w = w_sym_out)
...
def birth_cell(N,c,M,x_alive,x_fitness, x_cell, x_sym_in):
    ...
    if sum(x_fitness['cell']) != 0:
        ...
        x_cell_copy['w'][list(dead_cell_ids[:,0])] = x_cell_copy['w'][list(birth_cell_ids)]
        ...
        #x_sym_in_copy['w'][list(dead_cell_ids[:,0]), :] = x_sym_in_copy['w'][list(birth_cell_ids), :]
...
def birth_sym_in(N,c,M,x_alive_pre,x_alive_current,x_fitness, x_sym_in):
    ...
    for i in alive_cell_ids[:,0]:
        ...
        if np.sum(x_fitness['sym_in'][i,:]) != 0:
            ...
            #x_sym_in_copy['w'][i,list(dead_sym_in_ids)] = x_sym_in_copy['w'][i,list(birth_sym_in_ids)]
...
def birth_sym_out(N,c,M,x_alive_pre,x_fitness, x_sym_out):
    ...
    if np.sum(x_fitness['sym_out']) != 0:
        ...
        #x_sym_out_copy['w'][0,list(dead_sym_out_ids)] = x_sym_out_copy['w'][0,list(birth_sym_out_ids)]
...
def swap(N,c,M,x_alive,x_sym_in,x_sym_out,x_cell):
    ...
    w_in  = mean_or_zero(x_cell_copy['w'][np.where(x_alive_copy['cell']==1)])
    w_out = mean_or_zero(x_cell_copy['w'][np.where(x_alive_copy['cell']==1)])
...
def mutation_old(N,c,M,x_alive, x_cell, x_sym_in,x_sym_out,x_mut):
...
def mutation(N,c,M,x_alive, x_cell, x_sym_in,x_sym_out,x_mut, which_to_mutate_cell, which_to_mutate_sym):
    ...
    h_cell_min = x_mut['h_cell_min']
    h_cell_max = x_mut['h_cell_max']
    e_cell_min = x_mut['e_cell_min']
    e_cell_max = x_mut['e_cell_max']
    w_cell_min = x_mut['w_cell_min']
    w_cell_max = x_mut['w_cell_max']

    s_sym_in_min = x_mut['s_sym_in_min']
    s_sym_in_max = x_mut['s_sym_in_max']
    h_sym_in_min = x_mut['h_sym_in_min']
    h_sym_in_max = x_mut['h_sym_in_max']
    e_sym_in_min = x_mut['e_sym_in_min']
    e_sym_in_max = x_mut['e_sym_in_max']
    #w_sym_in_min = x_mut['w_sym_in_min']
    #w_sym_in_max = x_mut['w_sym_in_max']

    s_sym_out_min = x_mut['s_sym_out_min']
    s_sym_out_max = x_mut['s_sym_out_max']
    h_sym_out_min = x_mut['h_sym_out_min']
    h_sym_out_max = x_mut['h_sym_out_max']
    e_sym_out_min = x_mut['e_sym_out_min']
    e_sym_out_max = x_mut['e_sym_out_max']
    #w_sym_out_min = x_mut['w_sym_out_min']
   # w_sym_out_max = x_mut['w_sym_out_max']
    ...
    #delta_cell = np.random.normal(loc=0, scale= gamma_cell, size= n_mut_cell).reshape(n_mut_cell,1)
    #delta_cell = np.random.uniform(-gamma_cell, gamma_cell, n_mut_cell).reshape(n_mut_cell,1)
    
    

    for i in np.arange(n_mut_cell):
        i_trait = np.random.choice(which_to_mutate_cell)
        #print('')
        #print('i_mutants = ', i_mutants[i])
        #print('')
        if i_trait == 0:
            ...
            x_cell_copy['s'][i_mutants[i],0] =  delta_cell
        if i_trait == 1:
            a_trunc = h_cell_min 
            b_trunc = h_cell_max
            ...
            a, b = (a_trunc - loc) / scale, (b_trunc - loc) / scale
            delta_cell = truncnorm.rvs(a, b, loc = loc, scale = scale)
            x_cell_copy['h'][i_mutants[i],0] =  delta_cell
        if i_trait == 2:
            a_trunc = e_cell_min 
            b_trunc = e_cell_max
            ...
            a, b = (a_trunc - loc) / scale, (b_trunc - loc) / scale
            delta_cell = truncnorm.rvs(a, b, loc = loc, scale = scale)
            x_cell_copy['e'][i_mutants[i],0] =  delta_cell
        if i_trait == 3:
            a_trunc = w_cell_min 
            b_trunc = w_cell_max
    ...
    #print('Number of mutations inside hosts = ', n_mut_in)
    #delta_in = np.random.normal(loc=0, scale= gamma_sym_in, size= n_mut_in).reshape(n_mut_in,1)
    #delta_in = np.random.uniform(-gamma_sym_in,gamma_sym_in,n_mut_in).reshape(n_mut_in,1)
    
    for i in np.arange(n_mut_in):
        i_trait = np.random.choice(which_to_mutate_sym)
        ...
        if i_trait == 0:
            ...
            x_sym_in_copy['s'][i_mutants[i],j_mutants[i]] =  delta_in
        if i_trait == 1:
            ...
            a, b = (a_trunc - loc) / scale, (b_trunc - loc) / scale
            delta_in = truncnorm.rvs(a, b, loc = loc, scale = scale)
            x_sym_in_copy['h'][i_mutants[i],j_mutants[i]] =  delta_in
        if i_trait == 2:    
            ...
            a, b = (a_trunc - loc) / scale, (b_trunc - loc) / scale
            delta_in = truncnorm.rvs(a, b, loc = loc, scale = scale)
            x_sym_in_copy['e'][i_mutants[i],j_mutants[i]] =  delta_in
        #if i_trait == 3:    
        #    a_trunc = w_sym_in_min 
        #    b_trunc = w_sym_in_max
        #    loc = x_sym_in['w'][i_mutants[i],j_mutants[i]] 
        #    scale = gamma_sym_in
        #    a, b = (a_trunc - loc) / scale, (b_trunc - loc) / scale
        #    delta_in = truncnorm.rvs(a, b, loc = loc, scale = scale)
        #    x_sym_in_copy['w'][i_mutants[i],j_mutants[i]] =  delta_in
    ...
    #print('Number of mutations outside hosts = ', n_mut_out)
    #delta_out = np.random.normal(loc=0, scale= gamma_sym_out, size= n_mut_out).reshape(n_mut_out,1)
    delta_out = np.random.uniform(-gamma_sym_out,gamma_sym_out,n_mut_out).reshape(n_mut_out,1)
    ...
    for i in np.arange(n_mut_out):
        ...
        if i_trait == 0:
            ...
            x_sym_out_copy['s'][0,i_mutants_out[i]] =  delta_out
        if i_trait == 1:
            ...
            a, b = (a_trunc - loc) / scale, (b_trunc - loc) / scale
            delta_out = truncnorm.rvs(a, b, loc = loc, scale = scale)
            x_sym_out_copy['h'][0,i_mutants_out[i]] =  delta_out
        if i_trait == 2:
            ...
            a, b = (a_trunc - loc) / scale, (b_trunc - loc) / scale
            delta_out = truncnorm.rvs(a, b, loc = loc, scale = scale)
            x_sym_out_copy['e'][0,i_mutants_out[i]] =  delta_out
        #if i_trait == 3:
        #    a_trunc = w_sym_out_min 
        #    b_trunc = w_sym_out_max
        #    loc = x_sym_out_copy['w'][0,i_mutants_out[i]]
        #    scale = gamma_sym_out
        #    a, b = (a_trunc - loc) / scale, (b_trunc - loc) / scale
        #    delta_out = truncnorm.rvs(a, b, loc = loc, scale = scale)
        #    x_sym_out_copy['w'][0,i_mutants_out[i]] =  delta_out
...
def process_simulation_task(N,c,M,x_alive,D_cell,D_sym,w_for_cells, N_gen_max, R, s_for_symbionts, s_for_cells, N_sim, h_for_symbionts, e_for_symbionts, h_for_cells, e_for_cells, epsilon_cell, epsilon_sym, gamma_cell, gamma_sym, s_cell_max, h_cell_max, e_cell_max, w_cell_max, s_sym_max,h_sym_max,e_sym_max, which_to_mutate_cell, which_to_mutate_sym):
    
    
    s_cell = s_for_cells*np.ones((N,1))
    h_cell = h_for_cells*np.ones((N,1))
    e_cell = e_for_cells*np.ones((N,1))
    w_cell = w_for_cells*np.ones((N,1))

    x_cell = dict(s = s_cell, h = h_cell, e = e_cell, w = w_cell)

    s_sym_in = s_for_symbionts*np.ones((N,c)) 
    h_sym_in = h_for_symbionts*np.ones((N,c))
    e_sym_in = e_for_symbionts*np.ones((N,c))
    ...
    x_mut = dict(epsilon_cell = epsilon_cell,
        epsilon_sym_in = epsilon_sym,
        epsilon_sym_out = epsilon_sym, 
        gamma_cell = gamma_cell,
        gamma_sym_in = gamma_sym,
        gamma_sym_out = gamma_sym,
        s_cell_min = -s_cell_max, 
        s_cell_max = s_cell_max,
        h_cell_min = -h_cell_max,
        h_cell_max = h_cell_max,
        e_cell_min = -e_cell_max,
        e_cell_max = e_cell_max,
        w_cell_min = -w_cell_max,
        w_cell_max = w_cell_max,
        s_sym_in_min = -s_sym_max,
        s_sym_in_max = s_sym_max,
        h_sym_in_min = -h_sym_max,
        h_sym_in_max = h_sym_max,
        e_sym_in_min = -e_sym_max,
        e_sym_in_max = e_sym_max,
        s_sym_out_min = -s_sym_max,
        s_sym_out_max = s_sym_max,
        h_sym_out_min = -h_sym_max,
        h_sym_out_max = h_sym_max,
        e_sym_out_min = -e_sym_max,
        e_sym_out_max = e_sym_max)


    dfs = many_simulations_within_circle(N,c,M,x_alive,x_cell,x_sym_in,x_sym_out,D_cell,D_sym, x_mut, N_gen_max,R,N_sim, which_to_mutate_cell, which_to_mutate_sym)
    



    #df.to_csv('Data/e_sym_'+str(e_for_symbionts)+ '_h_sym_'+str(h_for_symbionts) + '_e_host_'+str(e_for_cells)+ '_h_host_'+str(h_for_cells) + '.csv', index =False)

    print('#################################################################################################################')
    print('')
    print('N = ', N, ', c = ',c,', M = ',M, ', D_cell = ',D_cell, ', D_sym = ', D_sym)
    print('')
    print('Number of generations max = ', N_gen_max,', R = ',R)
    print('')
    print('Traits for all cells: s = ',s_for_cells,', h = ',h_for_cells,', e = ',e_for_cells, ', w = ',w_for_cells)
    print('')
    print('Traits for symbions: h = ', h_for_symbionts,', e = ', e_for_symbionts, ', s = ',s_for_symbionts)
    print('')
    print('')
    print('Mutation variability: gamma_host = ', gamma_cell,', gamma_sym = ', gamma_sym)
    print('')
    print('')
    print('Mutation probability: epsilon_host = ', epsilon_cell,', epsilon_sym = ', epsilon_sym)
    print('')
    print('#################################################################################################################')
    
    #return(dfs['df_angle'])
    return(dict(df = dfs['df'], df_angle = dfs['df_angle']))
...
if __name__ == "__main__":
    ...
    h_for_cells_ls = n_args*[0]
    e_for_symbionts_ls = list(e_pairs[:,0])

    h_for_symbionts_ls = n_args*[0]
    ...
    # change n_args at the end of the next line if needed 
    args = [(N,c,M,x_alive,D_cell,D_sym,w_for_cells, N_gen_max,R,s_for_symbionts, s_for_cells, N_sim_ls[i], h_for_symbionts_ls[i], e_for_symbionts_ls[i], h_for_cells_ls[i], e_for_cells_ls[i], epsilon_cell_ls[i], epsilon_sym_ls[i], gamma_cell_ls[i], gamma_sym_ls[i], s_cell_max, h_cell_max, e_cell_max, w_cell_max, s_sym_max,h_sym_max,e_sym_max, which_to_mutate_cell, which_to_mutate_sym) for i in np.arange(n_args)]


    with multiprocessing.Pool(processes= num_processes) as pool:
        all_dfs = pool.starmap(process_simulation_task, args)

    end_time = time.time()

    #df = pd.concat(all_dfs)
    df = pd.concat([dfs['df'] for dfs in all_dfs])
    df_angle = pd.concat([dfs['df_angle'] for dfs in all_dfs])
